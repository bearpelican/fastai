...
def find_test_matches(elt, test_file:Path)->Tuple[List[Dict],List[Dict]]:
    ...
    direct_matches = direct_test_match(elt, lines, rel_path)
    fuzzy_matches = fuzzy_test_match(elt, lines, rel_path)
    return direct_matches, fuzzy_matches

def direct_test_match(elt, lines:List[Dict], rel_path:str)->List[TestFunctionMatch]:
...
def fuzzy_test_match(elt, lines:List[Dict], rel_path:str)->List[TestFunctionMatch]:
...
def _fuzzy_line_match(elt, lines)->List[TestFunctionMatch]:
    "Find any lines where `elt` is called"
    fn_name = nbdoc.fn_name(elt)
    result = []
    for idx,line in enumerate(lines):
...
def map_test(test_file, line, line_text):
    ...
    test_name = re.match(f'^def (test_\w*)', line_text).groups(0)[0]
