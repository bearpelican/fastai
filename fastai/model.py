...
class Stepper():
    ...
    def step(self, xs, y, epoch):
        ...
        if self.clip:   # Gradient clipping
            if self.fp16: nn.utils.clip_grad_norm_([p for p in self.m.parameters() if isinstance(p, torch.cuda.HalfTensor)], self.clip)
            elif IS_TORCH_04: nn.utils.clip_grad_norm_(trainable_params_(self.m), self.clip)
