...
def test_respects_import_order():
    docstr   = "`vision.learner`"
    expected = "[`vision.learner`](/vision.learner.html#vision.learner)"
    assert_link(docstr, expected, build_nb_cells(['fastai.text', 'fastai.vision']))

    # (AS) TODO: no longer need to test submodule collisions
...
def test_nb_module_name_has_highest_priority():
    ...
    docstr   = "`text.transform`"
    expected = "[`text.transform`](/text.transform.html#text.transform)"
    imports = """from fastai.text import *"""
    modules = gen_notebooks.get_imported_modules([gen_notebooks.get_code_cell(imports)], nb_module_name='fastai.text.transform')
    assert_link(docstr, expected, modules=modules)

    docstr   = "`tabular.transform`"
    expected = "[`tabular.transform`](/tabular.transform.html#tabular.transform)"
    imports = """from fastai.tabular import *"""
    modules = gen_notebooks.get_imported_modules([gen_notebooks.get_code_cell(imports)], nb_module_name='fastai.tabular.transform')
    assert_link(docstr, expected, modules=modules)
...
def test_link_vision_learner_priority():
    ...
    docstr   = "Pass in your `vision.data`, calculated `preds`, actual `y`,"
    expected = "Pass in your [`vision.data`](/vision.data.html#vision.data), calculated `preds`, actual `y`,"
